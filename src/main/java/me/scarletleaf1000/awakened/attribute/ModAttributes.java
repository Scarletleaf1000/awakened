package me.scarletleaf1000.awakened.attribute;
...
public class ModAttributes {
    ...
    public static final RegistryObject<Attribute> MINING_SPEED = ATTRIBUTES.register("mining_speed",
            () -> new RangedAttribute("attribute.name.awakened.mining_speed", 1.0D, 0.0D, 1024.0D)
                    .setSyncable(true));

    @SubscribeEvent
    public static void onEntityAttributeModification(EntityAttributeModificationEvent event) {
        event.getTypes().forEach(type -> {
            if (!event.has(type, MINING_SPEED.get())) {
                event.add(type, MINING_SPEED.get());
            }
        });
    }
}
